...
GET  /api/history?code=X&e=X → {history:[{t,c,g}], historyCoarse:[{t,c}], total, capacity, totalIn, totalOut, groups:[{id,name,count,totalIn,totalOut,opStats}]}  // g={groupId:count} ; history=fin 60s/48h ; historyCoarse=total seul 5min/60j
...
- **Historique par groupe** : point `{t, c, g:{[gid]:count}}` toutes les **60s** (`FINE_INTERVAL_MS`, était 30s → 2880 pts couvrent 48h au lieu de 24h, et moitié moins d'écritures DO). **Local** : `evt.history` (state.json) via `recordHistory()`. **Cloud** : clé DO **séparée `history`** (≠ `state`), écrite seulement par l'`alarm()` → `_save()` léger ; `_load()` migre l'ancien inline (waitUntil). `stats.html` trace `history[].g[gid]` (`spanGaps:true` ; `g` absent → `null`), affiché si >1 groupe.
